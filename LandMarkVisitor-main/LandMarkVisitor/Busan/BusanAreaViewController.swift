...
class BusanAreaViewController: UIViewController ,UITableViewDelegate , UITableViewDataSource{
    struct BusanAreaDataModel{
        var description : String
        var isExpand : Bool
    }
    
    var dataModels = [BusanAreaDataModel]()
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.BusanAreaLabel.text = dataModels[indexPath.row].description
        
        if dataModels[indexPath.row].isExpand == true {
            cell.BusanAreaLabel.numberOfLines = 0
        }else{
            cell.BusanAreaLabel.numberOfLines = 1
        }
    ...
    override func viewDidLoad() {
        ...
        for (_,value) in BusanArea.enumerated() {
            dataModels.append(BusanAreaDataModel.init(description: value, isExpand: false))
        }
